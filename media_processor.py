...
_MAX_SIDE_RATIO = 0.25
_MIN_RETAINED_AREA = 0.60
...
def detect_uniform_edges(
    ...
    *,
    ...
    min_border: int = 2,
    max_side_ratio: float = _MAX_SIDE_RATIO,
) -> EdgeCrop:
    ...
    if retained_area < _MIN_RETAINED_AREA:
        return EdgeCrop()
    return crop
...
def process_image(path: Path, *, dry_run: bool = False) -> ProcessResult:
        ...
        with Image.open(path) as source:
            ...
            if not crop.changed:
                return ProcessResult(
                    ...
                    reason="no safe uniform edge border",
                )
            box = crop.box(*original_size)
            output_size = (box[2] - box[0], box[3] - box[1])
            if dry_run:
                return ProcessResult(
                    path, True, crop, original_size, output_size, "dry run"
                )
    ...
    return ProcessResult(path, True, crop, original_size, output_size, "cropped")
...
def _video_crop_from_frames(
    ...
    crop = EdgeCrop(
        **{
            side: _consensus_value(
                [getattr(frame_crop, side) for frame_crop in frame_crops], required
            )
            for side in ("left", "top", "right", "bottom")
        }
    )

    # libx264 requires even dimensions. Crop less rather than risk subject pixels.
    left = crop.left - crop.left % 2
    top = crop.top - crop.top % 2
    right = crop.right - crop.right % 2
    bottom = crop.bottom - crop.bottom % 2
    if (width - left - right) % 2:
        right = max(0, right - 1)
    if (height - top - bottom) % 2:
        bottom = max(0, bottom - 1)
    crop = EdgeCrop(left, top, right, bottom)

    output_width = width - crop.left - crop.right
    output_height = height - crop.top - crop.bottom
    if output_width <= 0 or output_height <= 0:
        return EdgeCrop()
    if (output_width * output_height) / (width * height) < _MIN_RETAINED_AREA:
        return EdgeCrop()
    return crop


def process_video(path: Path, *, dry_run: bool = False) -> ProcessResult:
    ...
    with tempfile.TemporaryDirectory(prefix="media-crop-") as temp_dir:
        ...
        try:
            ...
            crop = _video_crop_from_frames(frame_paths, width, height)
    ...
    output_size = (box[2] - box[0], box[3] - box[1])
    if dry_run:
        return ProcessResult(path, True, crop, original_size, output_size, "dry run")
...
def process_media(path: Path, *, dry_run: bool = False) -> ProcessResult:
    """Process one supported image or video."""
    path = Path(path)
    suffix = path.suffix.lower()
    if suffix in IMAGE_EXTENSIONS:
        return process_image(path, dry_run=dry_run)
    if suffix in VIDEO_EXTENSIONS:
        return process_video(path, dry_run=dry_run)
